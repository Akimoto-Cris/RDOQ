...
def loadnetwork(archname, gpuid, act_bitwidth=-1):
    ...
    elif "deit" in archname: # or "vit" in archname
    ...
    elif "vit" in archname or "deit" in archname:
...
def loadrdcurves(archname,l,g,part,nchannelbatch=-1, Amse=False):
    if nchannelbatch>0:
        ...
        mat = io.loadmat(f'{archname}_nr_0011_ns_0064_nf_{nchannelbatch:04d}_rdcurves_channelwise_opt_dist_act{"_Amse" if Amse else ""}/{archname}_val_{l:03d}_{g:04d}_output_{part}')
    else:
        ...
        mat = io.loadmat(f'{archname}_nr_0011_ns_0064_rdcurves_channelwise_opt_dist_act/{archname}_val_{l:03d}_0064_output_{part}')
